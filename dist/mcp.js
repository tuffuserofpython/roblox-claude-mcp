...
export function buildMcpServer(store, defaultCmdTimeoutMs) {
    ...
    // ---- Read tools ------------------------------------------------------
    server.tool("roblox_status", "Check whether a Roblox game is currently connected to the bridge, plus basic counters.", {}, async () => json({
    ...
    server.tool("roblox_get_state", "Get the latest full game-state snapshot: players, positions, health, equipped tools, and metrics.", {}, async () => {
        const state = store.getState();
        if (!state)
            return json({ error: "no state received yet; is the Roblox script running?" });
        return json(state);
    });
    server.tool("roblox_query_events", "Query recent game events with optional filters. Returns newest-last. Use this to inspect player behavior and what happened in the game.", {
    ...
    server.tool("roblox_wait_events", "Long-poll for NEW events after a given seq. Blocks until events arrive or timeout. Use this to follow the game live with low latency.", {
    ...
    const dispatch = async (kind, args, wait, timeoutMs) => {
        if (!store.isConnected()) {
            return json({ error: "no Roblox game connected; commands would never run" });
        }
        const cmd = store.enqueueCommand(kind, args);
        if (!wait)
            return json({ queued: true, commandId: cmd.id });
        const result = await store.waitForResult(cmd.id, timeoutMs ?? defaultCmdTimeoutMs);
        if (!result)
            return json({ commandId: cmd.id, error: "timed out waiting for game to report result" });
        return json(result);
    };
    server.tool("roblox_run_luau", "Execute a Luau snippet on the Roblox SERVER and return its result. The snippet runs inside a function; use `return` to send a value back. Powerful: use for arbitrary inspection or mutation of the game.", {
    ...
    server.tool("roblox_set_property", "Set a property on an instance addressed by a game path, e.g. path 'Workspace.Part', property 'Transparency', value 0.5.", {
    ...
    }, async ({ path, property, value, wait }) => dispatch("set_property", { path, property, value }, wait ?? true));
    server.tool("roblox_create_instance", "Create a new instance under a parent path.", {
        ...
        wait: z.boolean().optional(),
    }, async ({ className, parentPath, properties, wait }) => dispatch("create_instance", { className, parentPath, properties: properties ?? {} }, wait ?? true));
    server.tool("roblox_destroy_instance", "Destroy an instance addressed by a game path.", {
        path: z.string(),
        wait: z.boolean().optional(),
    }, async ({ path, wait }) => dispatch("destroy_instance", { path }, wait ?? true));
    server.tool("roblox_fire_remote", "Fire a RemoteEvent. Set toAll=true to FireAllClients, or pass a userId to FireClient, otherwise it fires server-side handling.", {
        path: z.string().describe("Dotted path to the RemoteEvent."),
        args: z.array(z.any()).optional(),
        toAll: z.boolean().optional(),
        userId: z.number().optional(),
        wait: z.boolean().optional(),
    }, async ({ path, args, toAll, userId, wait }) => dispatch("fire_remote", { path, args: args ?? [], toAll: !!toAll, userId }, wait ?? false));
    server.tool("roblox_message", "Send a notification message to players (rendered by the script's message handler).", {
        text: z.string(),
    ...
    server.tool("roblox_snapshot", "Force the Roblox script to push a fresh state snapshot immediately, then return it.", {}, async () => {
        if (!store.isConnected())
            return json({ error: "no Roblox game connected" });
