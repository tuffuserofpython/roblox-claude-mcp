...
import type { CommandResult, GameEvent, GameState } from "./types.js";
import { DASHBOARD_HTML } from "./dashboard.js";

export interface BridgeOptions {
  port: number;
  host: string;
  /** Shared secret required in the x-roblox-key header. Empty disables auth. */
  authKey: string;
  /** Max seconds to hold a /poll long-poll open. */
 ...
 */
export function startBridge(store: Store, opts: BridgeOptions): http.Server {
...
async function handle(
  ...
  if (req.method === "POST" && path === "/events") {
    ...
    const latest = store.ingest(batch as Omit<GameEvent, "seq" | "ingestedAt">[]);
    return sendJson(res, 200, { ok: true, latestSeq: latest });
  }

  if (req.method === "POST" && path === "/state") {
    const body = (await readJson(req)) as GameState;
    store.setState(body);
  ...
  if (req.method === "POST" && path === "/results") {
    const body = await readJson(req);
    const arr: CommandResult[] = Array.isArray(body) ? body : body?.results ?? [body];
    for (const r of arr) if (r && r.id) store.recordResult(r);
    return sendJson(res, 200, { ok: true });
  }
