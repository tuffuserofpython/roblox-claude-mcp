...
export class Store {
  ...
  private pollWaiters: Array<(cmds: Command[]) => void> = [];
  /** Resolvers for event subscribers waiting for new events past a seq. */
  private eventWaiters: Array<{ afterSeq: number; resolve: (e: GameEvent[]) => void }> = [];
  ...
  ingest(raw: Omit<GameEvent, "seq" | "ingestedAt">[]): number {
    const now = Date.now();
    this.lastContact = now;
    for (const e of raw) {
      const ev: GameEvent = { ...e, seq: ++this.seq, ingestedAt: now };
      this.events.push(ev);
    }
  ...
  query(opts: {
    ...
    let out = this.events.filter((e) => {
      if (afterSeq != null && e.seq <= afterSeq) return false;
      if (types && !types.includes(e.type)) return false;
      if (userId != null && e.userId !== userId) return false;
      if (cutoff != null && e.ingestedAt < cutoff) return false;
      return true;
    });
  ...
  waitForEvents(afterSeq: number, timeoutMs: number): Promise<GameEvent[]> {
  ...
  isConnected(staleMs = 10000): boolean {
    return this.lastContact > 0 && Date.now() - this.lastContact < staleMs;
  }

  // ---- Commands --------------------------------------------------------

  enqueueCommand(kind: CommandKind, args?: Record<string, unknown>): Command {
    const cmd: Command = { id: randomUUID(), kind, args, queuedAt: Date.now() };
  ...
  waitForCommands(timeoutMs: number): Promise<Command[]> {
    if (this.commandQueue.length > 0) {
      return Promise.resolve(this.drainCommands());
    }
    return new Promise((resolve) => {
      const waiter = (cmds: Command[]) => resolve(cmds);
  ...
  private flushPollWaiters() {
    if (this.pollWaiters.length === 0 || this.commandQueue.length === 0) return;
  ...
  recordResult(res: CommandResult) {
  ...
  async waitForResult(id: string, timeoutMs: number): Promise<CommandResult | null> {
